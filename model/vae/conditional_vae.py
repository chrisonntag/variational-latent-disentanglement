...
class Encoder(keras.Model):
    ...
    def call(self, inputs, labels):
        reshaped_labels = tf.reshape(labels, [-1, 1, 1, self.label_dim]) # reshape to picture size (batch, width, height, channels)
        ones = tf.ones([inputs.shape[0]] + [self.input_dim[0], self.input_dim[1]] + [self.label_dim])  # fill with ones
        reshaped_labels = ones * reshaped_labels
        concat_inputs = tf.concat([inputs, reshaped_labels], axis=3)

        latent = self.model(concat_inputs)
...
class ConditionalVAE(keras.Model):
    ...
    def encode_dist(self, x, y):
        z_mean, z_log_var, _ = self.encode(x, y)
    ...
    def call(self, inputs, labels):
        self.z_mean, self.z_log_var = self.encode(inputs, labels)
